...
TXBIN_LD_LIBRARY=
TXBIN_LIBS="-lm -lpthread $LLVM_HOME/lib/libTxHooks.a -ldl -lxml2 -lgthread-2.0 -lgmodule-2.0 -lxml2 -lgobject-2.0 -lglib-2.0 -lpthread -lm -lstdc++ -pthread $LLVM_HOME/lib/libdonot-rt.a -lrt  -ldl -lpthread"

# OUTPUT: BC FILE NAME
TXBIN_OUTPUT_BC=${TXBIN_APP}.tx-cl.bc
TXBIN_OUTPUT_LL=${TXBIN_APP}.tx-cl.ll
TXBIN_OUTPUT_EXE=${TXBIN_APP}.tx-cl.exe
...
TXBIN_OPT_CMD="${LLVM_OPT} ${TXBIN_LLVM_LIBS} -tx-instrument  -musthook musthook.txt -log ${TXBIN_APP}.prof.out -ulc -ltf ltrd.txt  -o ${TXBIN_OUTPUT_BC} ${TXBIN_INPUT_BC}"
