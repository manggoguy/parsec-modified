...
TXBIN_OPT_CMD="${LLVM_OPT} ${TXBIN_LLVM_LIBS} -tx-instrument -log ${TXBIN_APP}.prof.out -musthook musthook.txt -ulc -ltf ltrd.txt -tx-tsan -o ${TXBIN_OUTPUT_BC} ${TXBIN_INPUT_BC}"
...
echo "${TXBIN_COMPILER} ${TXBIN_COMPILER_OPT} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}"
${TXBIN_COMPILER} ${TXBIN_COMPILER_OPT} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}
