...
TXBIN_LD_LIBRARY=
TXBIN_LIBS="$LLVM_HOME/lib/libTxHooks.a $LLVM_HOME/lib/libtsan_rtl.a $LLVM_HOME/lib/libtsan_icp.a $LLVM_HOME/lib/libtsan_san.a  -ldl -lpthread"

# OUTPUT: BC FILE NAME
TXBIN_OUTPUT_BC=${TXBIN_APP}.txtsan-cl.bc
TXBIN_OUTPUT_LL=${TXBIN_APP}.txtsan-cl.ll
TXBIN_OUTPUT_EXE=${TXBIN_APP}.txtsan-cl.exe
...
TXBIN_OPT_CMD="${LLVM_OPT} ${TXBIN_LLVM_LIBS} -tx-instrument -log ${TXBIN_APP}.prof.out -musthook musthook.txt -tx-tsan -ulc -ltf ltrd.txt -o ${TXBIN_OUTPUT_BC} ${TXBIN_INPUT_BC}"
...
echo "clang++ ${TXBIN_COMPILER_OPT} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}"
clang++ ${TXBIN_COMPILER_OPT} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}
