...
TXBIN_OPT_CMD="${LLVM_OPT} ${TXBIN_LLVM_LIBS} -tx-instrument -log ${TXBIN_APP}.prof.out -musthook musthook.txt -ulc -ltf ltrd.txt -o ${TXBIN_OUTPUT_BC} ${TXBIN_INPUT_BC}"
...
echo "${TXBIN_COMPILER} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}"
${TXBIN_COMPILER} -o ${TXBIN_OUTPUT_EXE} ${TXBIN_OUTPUT_BC} ${TXBIN_LIBS}
